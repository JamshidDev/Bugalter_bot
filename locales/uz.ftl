start_hi =Salom {$first_name}. Xush kelibsiz!
start_addition = 
    ⚡️Asosiy menu⚡️


    ❓ Sizda buxgalteriya bilan bog'liq muammolar bormi?
    ❓ Soliq tekshiruvlaridan charchadingizmi?
    ❓ Xato va kamchiliklar ko'payib ketdimi?

    ❗️ Endi bu muammo emas !!!
    ☝️ Biz sizga o'zimizning sifatli va hamyonbob xizmatlarimizni taklif qilamiz.

    🖇Bizning firma tadbirkorlar va buxgalterlar uchun soliq va moliyaviy hisobotlarni yuborishda amaliy yordam beradi!

    🧮Qaysi hisobot turini yubormoqchisiz, quyida oʼzingizga zarur boʼlgan boʼlimni tanlang

our_service_btn_title = ♻️ Bizning xizmatlar

about_us_btn_title = ℹ️ Biz haqimizda

about_us_text = ⚜️⚜️ <b>Biz haqimizda</b>⚜️⚜️
    Kompaniyamiz kichik va o'rta biznes uchun yuqori sifatli buxgalteriya xizmatlarini taklif etadi. Biz buxgalteriya hisobi, soliqni rejalashtirish, moliyaviy tahlil va hisobotlarni o'z ichiga olgan to'liq xizmatlarni taklif etamiz. Bizning tajribali buxgalterlar jamoasi sizning biznesingizni rivojlantirishga e'tibor qaratishingiz uchun buxgalteriya hisobingiz to'g'ri va o'z vaqtida bo'lishini ta'minlaydi. Xizmatlarimiz haqida ko'proq ma'lumot olish, shuningdek muvaffaqiyatga erishishingizda sizga yordam berishimiz uchun biz bilan bog'laning.


service_title = Siz qaysi <b>Soliq yoki moliyaviy hisobotini</b> yuborishimizni hohlaysiz? 
     

    <i>Quyidagilarni Tanlang! 👇</i> 



selected_service = Tanlangan xizmat turi: 🔰  <b> { $service_name} </b>
                    
    Hisobot yuborish uchun zarur fayl va ma'lumotlarni taqdim etishingiz lozim

start_go_title = Boshlash

task_key_title = 🔑 ETSP kalitini yuklang (YTT va firmalarga davlat hizmatlari markazidan beriladigan  2 ta fayl)
task_key_title_error = <b>⚠️ Noto'g'ri ma'lumot yuklandi</b>

    🔑 ETSP kalitini yuklang (YTT va firmalarga davlat hizmatlari markazidan beriladigan  2 ta fayl)
task_cer_error = ⚠️ <b>Noto'g'ri ma'lumot yuklandi</b>
    <i>🔑 EDSP sertifikatni yuklang </i> 

password_title = <b>🔐 Parolni kiriting</b>
password_title_error = ⚠️ <b>Noto'g'ri ma'lumot yuklandi</b>
     <i>🔐 Parolni kiriting</i> 

comment_title = 💬 Izoh yozing


order_create = <i>✅  Buyurtmangiz adminlarga jo'natildi iltimos adminlar javobini kuting. Adminlar 60 daqiqa ichida javob qaytarishadi.</i>

    🔰 Buyurtma raqami: <b>{ $order_number}</b>

expire_msg = 🛑 <b>Kutilmagan xatolik yuz berdi</b>
    <i>Itlimos qayta harakat qiling</i>

pricing_order_text = <b>Sizning buyurtmangiz qabul qilindi</b>
    ♻️ Buyurtma raqami : <b>{ $order_number}</b>
    💵 To'lov summasi: <b>{ $price} so'm</b>
    📞 Bog'lanish: <b>+998(94) 683-83-40</b>

    <i>Buyurtma to'lov amalga oshirilgandan keyin 12 soat ichida bajarilib sizga bot orqali xabar yuboriladi!</i>

change_language_title = <b>⚙️ Dastur tilini sozlash</b>
language_uz = Uzbek tili
language_ru = Rus tili

finished_order_title = <b>🏁 Buyurtma bajarildi</b>
    <i><b> {$order_number}</b> raqamli buyurtma muvofaqiyatli bajarildi ✅ </i>

reject_order_title =<b> 🛑 Sizning {$order_number} raqamli buyurtmangiz adminlar tomonidan raq etildi!</b>

    <b>💬 Sabab: </b> <i>{$comment}</i> 
    Bog'lanish uchun: <b>+998(94) 683-83-40</b>


stop_action =🔴 Bekor qilish

back_to_main_menu =◀️ Asosiy menu


// new version
main_menu_text_command = <i>⚡️Asosiy menu⚡️</i>
premium_service_name = 💰 Pullik xizmatlar
about_us_menu_name = ℹ️ Biz haqimizda
setting_menu_name = ⚙️ Sozlamalar
free_service_menu_name = 🆓 Xizmatlar
news_order_menu_name = 📥 Buyurtmalarim
change_language_title =🔄 Tilni o'zgartirish
system_lang_uz =🇺🇿 O'zbek tili
system_lang_ru =🇷🇺 Rus tili


free_service_ikpu_search = 🔍 IKPU  qidiruv

ikpu_search_text =🔍 Izlayotgan Debet va Kredit raqamlaringizni yozing!

    Masalan: <b>2810</b>-<b>2010</b>

invalid_search_msg = <i>⚠️ Noto'g'ri formatdagi xabar!</i>

     <i>☝️ Izlayotgan Debet va Kredit raqamlaringizni quyidagi formatda yozing!</i>
     Format: <b>2810</b>-<b>2010</b>

no_fount_bank_text =🧐 Siz kiritgan debet va kredit raqamlar bo'yicha hech narsa topilmadi


loading_bank_text = 🔍 Qidirilmoqda...

no_premium_service = Vaqtinchalik pullik xizmatlar yopilgan!
premuim_service_list_title = 💰 Pullik xizmatlar!
    Kerakli xizmat turini tanlang

select_free_service =👇 Kerakli funksiyani tanlang!


bank_result_message_text = <b>🎊Topilgan ma'lumotlar🎊</b>

    Debet: <b>{$debet}</b>
    Kredit: <b>{$kredit}</b>
    O'tkazma: <b>{$result}</b>






back_to_setting_btn = 🔙 Sozlamalar